...
ENERGY_PER_LITER_HOT_WATER = 0.0516   # Assuming a temperature difference of about 30°C
BASELINE_ELECTRICITY_KWH_PER_5MIN = 0.025  # 300W continuous
...
class NPC:
    ...
    def _get_duration_from_config(self, device_name, device_type):
        ...
        if device_info and "typical_duration" in device_info["device"]:
            ...
            if device_type == "electricity":
                # Assume typical_duration in hours for electricity devices
                duration_hours = random.uniform(min_duration, max_duration)
                duration_seconds = duration_hours * 3600
            elif device_type == "water":
                # Assume typical_duration in minutes for water devices
    ...
    def finish_action(self):
        if self.current_action and self.state == "Performing Action":
            ...
            # Reset state and decide next action
            self.current_action = None
            self.action_end_time = None
            self.state = "Idle"
