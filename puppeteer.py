...
def standardize_timestamp_format(timestamp):
    """Convert a timestamp to a standardized ISO format with a colon in the timezone offset."""
    try:
        # Ensure the timezone offset has a colon (e.g., +0100 -> +01:00)
        if len(timestamp) > 5 and (timestamp[-5] in ['+', '-']) and ':' not in timestamp[-5:]:
            timestamp = timestamp[:-2] + ':' + timestamp[-2:]
        # Replace 'T' with a space if present and parse the timestamp
...
def get_solar_grid_consumption(solar_production, total_electr_consumption):
    ...
    return grid_consumption, filtered_total_electr_consumption
...
def generate_output(houseID, solar_production, electricity_consumption, water_consumption, device_consumption, solar_grid_consumption, battery_data, device_statistical_data, temperature, humidity, air_quality, air_quality_description, output_dir="./sim_result"):
    """
        ...
        ts_data = {
            ...
            "energy_management_sensors": {
                "solar_power": {
                    ...
                    "electricity_consumption": electricity_consumption_std[ts]
                },
                "battery": {
    ...
    output_file = f"{output_dir}/{houseID}_output.json"
    with open(output_file, "w") as f:
        json.dump(output_data, f, indent=4)

    print(f"Generated output file: {output_file}", flush=True)


def complete_simulation_generate(name_of_config_file: str = "config_default.json", output_dir: str = "./sim_result"):
    """
    Run the complete simulation and generate the output file.
    """
    # Import the configuration file
    with open(name_of_config_file) as f:
        config = json.load(f)
    
    type_of_simulation = config["basic_parameters"]["type_of_simulation"]["type"]  # Just fast_forward for now
    ...
    if type_of_simulation == "fast_forward":
        ...
        print("\033[92mStarting fast forward simulation...\033[0m", flush=True)
        print("*" * 100, flush=True)
        ...
        grid_consumption = get_solar_grid_consumption(solar_production=solar_prod,
                                                    total_electr_consumption=total_consumption[0])[0]
        ...
        print("Generating output file...", flush=True)
        generate_output(
        ...
        print("\033[92mOutput file generated correctly\033[0m", flush=True)
        
        print("\033[92mSimulation completed successfully!\033[0m", flush=True)
    
    elif type_of_simulation == "real_time":
         
        print("*" * 100, flush=True)
        print("\033[92mStarting real time simulation...\033[0m", flush=True)
        print("*" * 100, flush=True)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        print("No configuration file or output directory provided. Using default configuration and output.", flush=True)
        complete_simulation_generate()
    elif len(sys.argv) == 2:
        print(f"Using configuration file: {sys.argv[1]}", flush=True)
        config_file = sys.argv[1]
        complete_simulation_generate(config_file)
    elif len(sys.argv) == 3:
        print(f"Using configuration file: {sys.argv[1]}", flush=True)
        print(f"Output directory: {sys.argv[2]}", flush=True)
        config_file = sys.argv[1]
        output_dir = sys.argv[2]
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        complete_simulation_generate(config_file, output_dir)  # Pass output_dir
    else:
        print("Usage: python puppeteer.py [config_file] [output_directory]", flush=True)
        exit(1)
